...
function companyMarker(item, column, inPopover = false) {
  ...
  return `<button class="cycle-marker${mine}${est}${inPopover ? " in-popover" : ""}" data-open-details="${esc(item.company)}"${column.future ? ` data-estimate="${esc(column.label)} ${column.year}"` : ""} aria-label="${esc(tip.join(", "))}">
...
function openDetails(company, estimateWhen = "") {
  const item = findOpening(company);
  track("opening_view");
  const match = openingMatch(item);
