...
|---|---|---|
| P-14 | No stated minimum age anywhere; minors' obligations unassessed | **Researched 3 Sep 2026, one action open.** Written up in [MINORS-AND-AGE.md](MINORS-AND-AGE.md). COPPA risk is low — Promptly is not directed to under-13s by any reasonable reading — but the app states **no minimum age at all**, which is the first thing a school's counsel or an App Store reviewer looks for. The realistic case is a 16–17-year-old, which falls under state minor-protection law rather than COPPA and needs current legal advice. FERPA does not bind Promptly today and becomes a contract question at the first school deal. **Wording applied 3 Sep 2026** with Marley's sign-off: Terms now carry a "Who can use Promptly" section stating 16+ and the under-13 undertaking, and the privacy page carries a "Children" section. Pinned by `tests/privacy.test.js` so a later copy edit cannot quietly drop it. The open item is no longer code — it is legal review before the first school contract. |
| P-12 | Unthrottled send endpoint and unauthenticated analytics writes | **Closed 3 Sep 2026.** `/api/send-test` pushes to a real device and was the only one of the three send endpoints with no rate limit; it now shares `takeTestAlertSlot` with send-alert and send-recap. `/api/stats` accepts an anonymous POST that increments counters — capped per requester (hashed, so throttling does not create the per-visitor identifier the analytics design avoids), failing open so a limiter fault never loses real signal. |
| P-13 | No standing check that server secrets stay out of the browser | **Closed 3 Sep 2026.** Audited clean: no server-only variable is assigned in any client file, no secret-shaped literal appears in one, `build-web.js` inlines nothing, and git history contains no committed key — only `.env.example` placeholders. That matters because the repo is public, where a key committed once is exposed permanently. Now enforced by `tests/no-client-secrets.test.js` across 17 client files, verified to catch a named assignment, a bare key value, and a `process.env` read. |
| P-11 | "Unconfirmed profiles are deleted after 14 days" was not true for the case it describes | **Closed 3 Sep 2026.** `purgeUnverified()` walks Upstash subscriber records, but `/api/subscribe` requires a CONFIRMED email — so a student who signs up and never clicks the link never gets an Upstash record, and there was nothing for the purge to find. Their Supabase account persisted indefinitely while the banner told them otherwise. A daily sweep now deletes unconfirmed Supabase accounts older than 14 days, guarded on three confirmation fields, vetoed by any confirmed Upstash subscriber, vetoed again if that check errors, and capped at 50 per run. Counts are shown on /admin.html. |
| P-09 | Exact school, graduation year, major, and interests are duplicated in Supabase account metadata and the Upstash alert copy | **Partly closed 3 Sep 2026.** `serverAlertProfile()` was a wholesale spread of `accountProfile()`, so the alert store received all four. `major` and `interests` are read by nothing server-side — not `matchesOpening()`, not any dashboard — so they are no longer sent, and the daily job scrubs existing records. Both remain in Supabase metadata, which is what makes a profile follow you to a new device. Graduation year is now sent as a coarse band rather than the exact year (option 2, chosen 3 Sep) — see below. School is still duplicated, which is the deliberate remaining trade: it is the field the school pilot conversations actually need. **The Supabase half is not a leak:** the server never reads `user_metadata`, and account deletion removes the Supabase user, taking the metadata with it. What it was missing was the abandoned-signup case — see P-11. |
