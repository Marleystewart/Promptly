// Citizens' branded TalentBrew site publishes live search-result fragments as
// unauthenticated JSON. The aggregator performs the strict student-role gate.

const cheerio = require("cheerio");

const ORIGIN = "https://jobs.citizensbank.com";
...
async function fetchPage(term, page) {
  ...
  if (!res.ok) throw new Error(`${res.status} citizens`);
...
function parsePage(html) {
  ...
  $("#search-results-list li").each((_, item) => {
    ...
    const location = link.find(".job-location, .loc").text().replace(/\s+/g, " ").trim();
