...
// Some Phenom sites load results from a POST to <origin>/widgets with
// ddoKey:"refineSearch" instead of server-rendering phApp.ddo. Same return
// shape: plain { title, url, location } for aggregator.js.
    ...
    for (let from = 0; from < 100; from += 10) {
      ...
      try {
        const res = await fetch(`${origin}/widgets`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "application/json", Origin: origin, Referer: `${origin}/us/en/search-results`, "User-Agent": "Mozilla/5.0 (compatible; PromptlyJobs/1.0)" },
          body: JSON.stringify({ lang: "en_us", deviceType: "desktop", country: "us", ddoKey: "refineSearch", sortBy: "Most Recent", subsearch: "", from, jobs: true, counts: true, all_fields: [], size: 10, clearAll: false, jdsource: "facets", isSliderEnable: false, pageName: "search-results", siteType: "external", keywords: term, global: true, selected_fields: {}, locationData: {} }),
          signal: AbortSignal.timeout(12000),
        });
      ...
      for (const job of jobs) {
        const title = String(job.title || "").replace(/\s+/g, " ").trim();
        const applyUrl = String(job.applyUrl || job.imApplyUrl || "").trim();
        const location = String(job.cityStateCountry || job.cityState || job.location || "").replace(/\s+/g, " ").trim();
        if (title && /^https:\/\//i.test(applyUrl)) seen.set(applyUrl, { title, url: applyUrl, location });
      }
